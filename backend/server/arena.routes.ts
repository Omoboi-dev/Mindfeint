...
arenaRoutes.post("/round", async (req, res) => {
  try {
    ...
    const round = await createDetectorRound(packId, voterId);
    res.json(round);
  } catch (e: any) {
...
// Hider mode: get a prompt to answer.
arenaRoutes.get("/prompt", (req, res) => {
  const packId = String(req.query?.pack ?? "spicy");
  res.json(pickHiderPrompt(packId));
});

// Hider mode: submit a human answer (tied to the prompt shown) to seed future rounds.
arenaRoutes.post("/hide", (req, res) => {
  ...
  res.json(submitHiderAnswer(prompt, text, voterId));
});
